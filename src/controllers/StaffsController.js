...
class StaffsController {
  async loginStaff(req, res) {
    try {
      ...
      let loginRespone = await staffsService.loginStaff(phone, password);
  ...
  async getStaffById(req, res) {
    ...
    try {
      ...
      const staff = await staffsService.getStaffById(id, req.get("Host"));
  ...
  async getStaffsWithWage(req, res) {
    try {
      const listStaffsByFilter = await staffsService.getStaffsWithWage(
        req.query,
        req.get("Host")
      );
      if (listStaffsByFilter.length === 0)
  ...
  async createStaff(req, res) {
    ...
    try {
      ...
      const data = {
        ...
        imagePath: req.file
          ? req.get("Host") + "/src/images/staffs/" + req.file.filename
          : "",
  ...
  async updateStaff(req, res) {
    try {
      ...
      let basicWage = req.body.basicWage
        ? parseFloat(req.body.basicWage)
        : undefined;
      ...
      if (
        (roleId != undefined || departmentId != undefined) &&
        roleIdAuth != variable.AdminRoleId
      ) {
        if (req.file) deleteImgByPath(req.file.path);
        return res
          ...
          .send(
            "No permission! Update role or department only works for admin account!"
          );
      }
      if (basicWage != undefined && roleIdAuth != variable.AdminRoleId) {
        ...
        return res
          .status(variable.Forbidden)
          .send(
            "No permission! Update basic wage only works for admin account!"
          );
      }
      date.setHours(date.getHours() + 7);
      data = {
      ...
      let update = await staffsService.updateStaff(id, data, req.get("Host"));
  ...
  async changePassword(req, res) {
    try {
      ...
      let changePassword = await staffsService.changePassword(
        id,
        data,
        roleIdAuth
      );
  ...
  async deleteManyStaffs(req, res) {
    try {
      ...
      if (delManyStaffs == variable.NotFound)
        return res.status(variable.NotFound).send("No staffs is deleted");
